    ...
    override public func extract(text: String, ref: Date, match: NSTextCheckingResult, opt: [OptionType: Int]) -> ParsedResult?
	{
		...
		else if match.isNotEmpty(atRangeIndex: dayGroup1)
		{
            ...
            if day1 == "明" || day1 == "聽"
			{
                ...
                if refMoment.hour > 1 { moment = moment.added(1, .day) }
            } else if day1 == "昨" || day1 == "尋" || day1 == "琴" { moment = moment.added(-1, .day) }
            
            if time1 == "早" || time1 == "朝"
			{
                result.start.imply(.hour, to: 6)
				result.start.imply(.minute, to: 0)
				result.end!.imply(.hour, to: 11)
				result.end!.imply(.minute, to: 59)
            }
			else if time1 == "晚"
			{
				...
				result.end!.imply(.hour, to: 5)
				result.end!.imply(.minute, to: 59)
				result.end!.imply(.second, to: 59)
            }
        ...
        } else if match.isNotEmpty(atRangeIndex: timeGroup2)
		{
			...
			{
                result.start.imply(.hour, to: 13)
                result.start.imply(.meridiem, to: 1)
				result.end!.imply(.hour, to: 17)
				result.end!.imply(.minute, to: 59)
				result.end!.imply(.second, to: 59)
				result.end!.imply(.meridiem, to: 1)
            } else if time2 == "中"
        ...
        } else if match.isNotEmpty(atRangeIndex: dayGroup3)
		{
            ...
            if day3 == "明" || day3 == "聽"
			{
                // Check not "Tomorrow" on late night
                if refMoment.hour > 1 { moment = moment.added(1, .day) }
            } else if day3 == "昨" || day3 == "尋" || day3 == "琴" { moment = moment.added(-1, .day)  }
        ...
        result.start.assign(.day, value: moment.day)
        result.start.assign(.month, value: moment.month)
        result.start.assign(.year, value: moment.year)
		result.end!.assign(.day, value: moment.day)
		result.end!.assign(.month, value: moment.month)
		result.end!.assign(.year, value: moment.year)
