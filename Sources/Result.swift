...
public struct ParsedResult
{
    ...
    public init(ref: Date, index: Int, text: String, tags: [TagUnit: Bool] = [TagUnit: Bool](), start: [ComponentUnit: Int]? = nil, end: [ComponentUnit: Int]? = nil, isMoveIndexMode: Bool = false)
	{
        ...
        if let end = end { self.end = ParsedComponents(components: end, ref: ref) }
...
public struct ParsedComponents
{
	...
	{
        if let components = components { knownValues = components }
    ...
    public var date: Date { return moment }
    
    public var moment: Date
	{
        var comps = DateComponents()
        
        if let year = self[.year] { comps.year = year }
        if let month = self[.month] { comps.month = month  }
        if let day = self[.day] { comps.day = day  }
        if let hour = self[.hour] { comps.hour = hour }
        if let minute = self[.minute] { comps.minute = minute }
        if let second = self[.second] { comps.second = second }
        if let millisecond = self[.millisecond] { comps.nanosecond = millisecondsToNanoSeconds(millisecond) }
        ...
        if Chrono.sixMinutesFixBefore1900 && newDate.utcYear < 1900 { return newDate.added(6, .minute) }
